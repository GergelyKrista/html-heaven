...
export function SubmitForm() {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.name.endsWith(".html") && !file.name.endsWith(".htm")) {
      setError("HTML files only."); return;
    }
    setError("");
    setHtmlFile(file);
    ...
    <div className="mx-auto max-w-xl">
      ...
      {step === "upload" && (
        <div className="space-y-4">
          <div
            ...
            className="cursor-pointer rounded-lg border border-dashed border-border bg-surface px-6 py-10 text-center transition-colors hover:border-border-light"
          >
            {htmlFile ? (
              <>
                <p className="text-[14px] font-medium">{htmlFile.name}</p>
                <p className="mt-0.5 text-[12px] text-muted">{(htmlFile.size / 1024).toFixed(1)} KB</p>
              </>
            ) : (
              <>
                <p className="text-[14px] font-medium">Choose an HTML file</p>
                <p className="mt-0.5 text-[12px] text-muted">Single file, max 2MB</p>
              </>
            )}
          </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              maxLength={100} placeholder="My App"
          ...
          <div>
            ...
            <textarea
              ...
              maxLength={1000} rows={3} placeholder="What does it do?"
          ...
          <div className="flex gap-2">
            <button onClick={() => setStep("upload")} className="h-9 flex-1 rounded-lg border border-border text-[13px] font-medium text-muted-light hover:text-foreground">
              Back
            </button>
            <button
      ...
      {step === "preview" && (
        <div className="space-y-4">
            ...
            {selectedTags.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {selectedTags.map((tag) => (
                  <span key={tag} className="rounded-md bg-surface-2 px-2 py-0.5 text-[11px] text-muted">{tag}</span>
                ))}
              </div>
            )}
          </div>
          {error && <p className="text-[12px] text-red-400">{error}</p>}
          <div className="flex gap-2">
            <button onClick={() => setStep("metadata")} className="h-9 flex-1 rounded-lg border border-border text-[13px] font-medium text-muted-light hover:text-foreground">
              Back
            </button>
            <button onClick={handleSubmit} className="h-9 flex-1 rounded-lg bg-primary text-[13px] font-semibold text-white hover:bg-primary-hover">
              Submit for review
            </button>
          </div>
        </div>
